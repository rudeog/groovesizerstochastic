...
seqSetTransportState(uint8_t state)
{
   ...
   // non slave mode - create an initial tick
   if (state == TRANSPORT_STARTING && !SLAVE_MODE()) {
      seqClockTick(); 
   }
}
...
seqClockTick(void)
{
   if(gRunningState.transportState == TRANSPORT_STARTING)
